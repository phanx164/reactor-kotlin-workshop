...
class c4_LifecycleHooks : LifecycleHooksBase() {
  ...
  fun no_subscription_no_gains() {
    ...
    val temperatureFlux = room_temperature_service() //todo: change this line only
      .doOnSubscribe { hooksTriggered.add("subscribe") }
  ...
  fun atomic_counter() {
    ...
    val temperatureFlux = room_temperature_service() //todo: change this line only
      .doOnNext { counter.getAndIncrement() }
  ...
  fun ordering_is_important() {
    ...
    val orderOfExecution = Arrays.asList("one", "two", "three") //todo: change this line only
  ...
  fun one_to_rule_them_all() {
    ...
    val flux = Flux.just(1, 2, 3) //todo: change this line only
      .doOnEach { signals.add (it.type.name)}
