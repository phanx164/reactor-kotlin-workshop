...
class c1_Introduction : IntroductionBase() {
  ...
  fun hello_world() {
    ...
    val result: String? = serviceResult.block() //todo: change this line only
  ...
  fun unresponsive_service() {
    val exception: Exception = assertThrows<IllegalStateException> {
      ...
      val result: String? = serviceResult.block(Duration.ofSeconds(1)) //todo: change this line only
    }
  ...
  fun empty_service() {
    ...
    val optionalServiceResult: Optional<String> = serviceResult.blockOptional()//todo: change this line only
    Assert.assertTrue(!optionalServiceResult.isPresent)
    Assert.assertTrue(emptyServiceIsCalled.get())
  }
  ...
  fun multi_result_service() {
    ...
    val result: String = serviceResult.blockFirst().toString() //todo: change this line only
    Assert.assertEquals("valid result", result)
  }
  ...
  fun fortune_top_five() {
    ...
    val results = serviceResult.collectList().block() //todo: change this line only
    Assert.assertEquals(
      Arrays.asList(
        "Walmart",
        "Amazon",
    ...
    Assert.assertTrue(fortuneTop5ServiceIsCalled.get())
  }
  ...
  fun nothing_happens_until_you_() {
    val companyList: CopyOnWriteArrayList<String> = CopyOnWriteArrayList<String>()
    val serviceResult: Flux<String> = fortuneTop5()
    serviceResult
  ...
  fun leaving_blocking_world_behind() {
    val serviceCallCompleted: AtomicReference<Boolean> = AtomicReference<Boolean>(false)
    val companyList: CopyOnWriteArrayList<String> = CopyOnWriteArrayList<String>()
    ...
    Thread.sleep(1000)
    Assert.assertTrue(serviceCallCompleted.get())
    Assert.assertEquals(
      Arrays.asList(
        "Walmart",
        "Amazon",
