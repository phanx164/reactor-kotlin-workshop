...
class c2_TransformingSequence : TransformingSequenceBase() {
  ...
  fun cast() {
    val numbersFlux = object_service()
      .cast(String::class.java) //todo: change this line only
    StepVerifier.create(numbersFlux)
  ...
  @Test
  fun maybe() {
    val result = maybe_service() //todo: change this line
  ...
  fun sequence_starts_with_zero() {
    val result = Mono.just(0).concatWith(numerical_service()) //todo: change this line only
